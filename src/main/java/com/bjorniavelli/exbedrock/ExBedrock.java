...
//        for (int i = 0; i < ExDrop.MAX_HARDNESS_LEVEL; i++)
//        {
//            // START HERE!
//            // Process each level.
//            // It'll look something like 'ProcessExDrop(int level, config.getStringList("exUnnamedDrops", "" + i, new String[]...
//            // Wait, we have to do these manually, because we need defaults, no for loop.
//            // Defaults Stink.  >_>
//        }

        for (String toolName : exDropToolTypes)
        {
            ConfigCategory toolCat = config.getCategory("exDrop" + toolName);
            // Start here
            // Change the exXXXDrops to a single HashMap(?), and key in the array of ExDrops to the toolname.

            for (String toolLevel : toolCat.keySet())
            {
                // Basically... ???
                // Need to modify ExDrop class to compensate for th enew changes.
            }
        }

//        readDrops (config);
//        processExDropsCategories(config);

        config.save();
    }

//    public void processExDropsCategories(Configuration config)
//    {
...
//    public void createDefaultConfig(Configuration config)
//    {
//        config.load();
//
//        // Which block drops stuff
//        config.get("general", "ExBlock", "minecraft:bedrock");
//
//        // Empty Hand
////        config.get("ExDropsEmptyHand", "hardnessLevel", "0");
////        config.get("ExDropsEmptyHand", "type", "empty");
////        config.get("ExDropsEmptyHand", "hardness", "minecraft:dirt");
////        config.get("ExDropsEmptyHand", "exDrops", {"minecraft:stick");
//
//
//        config.save();
//    }
